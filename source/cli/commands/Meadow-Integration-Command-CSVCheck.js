...
class QuackageCommandCSVCheck extends libCommandLineCommand
{
	constructor(pFable, pManifest, pServiceHash)
	{
		...
		this.options.Aliases.push('csv_c');
		this.options.Aliases.push('csv_check');
		...
		this.options.CommandOptions.push({ Name: '-r, --records', Description: 'Output the full record dump of the CSV file in the statistics object.'});
	...
	onRunAsync(fCallback)
	{
		...
		{
			tmpOperationState.RawOutputFile = `${process.cwd()}/CSV-Stats-${libPath.basename(tmpOperationState.RawInputFile)}.json`;
