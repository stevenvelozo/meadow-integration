...
class QuackageCommandTSVTransform extends libCommandLineCommand
{
	constructor(pFable, pManifest, pServiceHash)
	{
		...
		this.options.CommandOptions.push({ Name: '-o, --output [filepath]', Description: 'The comprehension output file.  Defaults to ./TSV-Comprehension-[filename].json'});
...
/* Comprehension Mapping File (for the file `/debug/testdata/airports.tsv` in this repository):
{
	"Entity": "Airport",
	...
	"Mappings":
	{
		"Code": "{~D:iata~}",
		"Name": "{~D:name~}",
		"Description": "{~D:name~} airport in {~D:city~} auto-ingested from TSV file.",
		"City": "{~D:city~}",
		"State": "{~D:state~}",
		"Country": "{~D:country~}",
		"Latitude": "{~D:lat~}",
...
quack tsvtransform testdata/airports.tsv -e Airport -n "GUIDAirport" -g "Airport-{~D:iata~}"" -c "Code={~D:iata~},Name={~D:name~},Description={~D:name~} airport in {~D:city~} auto-ingested from TSV file.,City={~D:city~},State={~D:state~},Country={~D:country~},Latitude={~D:lat~},Longitude={~D:long~}"
 ...
 *
 * This will generate a comprehension from a TSV file.
 * 
 ...
 * - Mappings: The mappings for the comprehension.  if not provided, will be generated from the first row of the TSV.
 */
	generateMappingFromRecord(pFileName, pRecord)
	{
		...
		// For instance "my favorite cats.tsv" would become "MyFavoriteCats"
	...
	onRunAsync(fCallback)
	{
		...
		{
			tmpOutputFileName = `${process.cwd()}/TSV-Comprehension-${libPath.basename(tmpFile)}.json`;
		...
		{
			tmpInputFileName = `${process.cwd()}/TSV-Comprehension-${libPath.basename(tmpFile)}.json`;
...
//						Mappings: false
		...
		if (!this.fable.FilePersistence.existsSync(tmpFile))
		{
			...
			if (!this.fable.FilePersistence.existsSync(tmpFile))
			{
				this.fable.log.error(`File [${tmpFile}] does not exist in the current working directory.  Could not parse input TSV file.  Aborting.`);
				return fCallback();
			}
		}
		...
		this.fable.log.info(`Parsing TSV file [${tmpFile}]...`);
...
module.exports = QuackageCommandTSVTransform;